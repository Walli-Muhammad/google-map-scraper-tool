...
def run_scraper(search_query: str = SEARCH_QUERY, max_scrolls: int = MAX_SCROLLS, headless: bool = HEADLESS):
    ...
    # ── 5. Save results ───────────────────────────────────────────────────
    if results:
        df = pd.DataFrame(results, columns=["Clinic Name", "Website", "Phone"])
        df.to_csv(OUTPUT_FILE, index=False, encoding="utf-8-sig")
        print(f"\n{'='*50}")
        print(f"✅  Done! {len(df)} record(s) saved to '{OUTPUT_FILE}'.")
        print(f"{'='*50}")
        return df
    else:
        print("\n⚠  No results were collected. The CSV was not written.")
        return pd.DataFrame()
